...
+ (QLCoreTextData *)parseContent:(NSString *)content config:(QLFrameParserCfg*)config {
    ...
    // 将生成好的 CTFrameRef 实例和计算好的绘制高度保存到 QLCoreTextData 实例中，最后返回 QLCoreTextData 实例
    QLCoreTextData *data = [[QLCoreTextData alloc] init];
    data.ctFrame = frame;
...
#pragma mark - 从如上格式的模版文件中生成CoreTextData
// 方法一:用于提供对外的接口，调用方法二实现从一个 JSON 的模版文件中读取内容，然后调用方法五生成CoreTextData
+ (QLCoreTextData *)parseTemplateFile:(NSString *)path config:(QLFrameParserCfg*)config {
    ...
    QLCoreTextData *data = [self parseAttributedContent:content config:config];
    data.imageArray = imageArray;
    data.linkArray = linkArray;
    return data;
}

// 方法二:读取 JSON 文件内容，并且调用方法三获得从NSDictionary到NSAttributedString的转换结果。
        ...
        if ([array isKindOfClass:[NSArray class]]) {
            for (NSDictionary *dict in array) {
                ...
                if ([type isEqualToString:@"txt"]) {
                    ...
                    // 创建 QLCoreTextImage
                    QLCoreTextImage *imageData = [[QLCoreTextImage alloc] init];
                    ...
                    [self parseAttributedContentFromNSDictionary:dict config:config];
                    [result appendAttributedString:as];
                    // 创建 CoreTextLinkData
                    NSUInteger length = result.length - startPos;
                    NSRange linkRange = NSMakeRange(startPos, length);
                    QLCoreTextLink *linkData = [[QLCoreTextLink alloc] init];
...
// 方法三:NSDictionary内容转换为NSAttributedString
...
+ (QLCoreTextData *)parseAttributedContent:(NSAttributedString *)content config:(QLFrameParserCfg*)config {
    ...
    // 将生成好的 CTFrameRef 实例和计算好的缓制高度保存到 QLCoreTextData 实例中，最后返回 QLCoreTextData 实例
    QLCoreTextData *data = [[QLCoreTextData alloc] init];
    data.ctFrame = frame;
    data.height = textHeight;
...
+ (CTFrameRef)createFrameWithFramesetter:(CTFramesetterRef)framesetter config:(QLFrameParserCfg *)config height:(CGFloat)height {
...
+ (NSAttributedString *)parseImageDataFromNSDictionary:(NSDictionary *)dict config:(QLFrameParserCfg*)config {
